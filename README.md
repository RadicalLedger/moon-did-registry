---
title: 'Moonbase DID Registry'
index: 0
category: 'moon-did-registry'
type: 'reference'
source: 'https://github.com/uport-project/moon-did-registry/blob/develop/README.md'
---

# Moonbase DID Registry

This library contains the Moonbase contract code that allows the owner of an moon-did identity to update the attributes
...
This contract allows Moonbase addresses to present signing information about themselves with no prior registration. It
...
|-----------------|-----------------|------------|------------|----------------------------------------------------------------------------------------------------------------------------------------|
| Mainnet         | mainnet         | 1284         | 0x507        | [0xeCCF09329968e51aC509500D95f171E40AAE0F3B](https://moonscan.io/address/0xeCCF09329968e51aC509500D95f171E40AAE0F3B)                  | 
| Moonbase Alpha         | alpha         | 1287         | 0x504        | [0x1205411db2cC925253A57299f3313075DDD48439](https://moonbase.moonscan.io/address/0x1205411db2cC925253A57299f3313075DDD48439)          | 
...
To use the contract, we provide hardhat artifacts. Once you require the `moon-did-registry` module, you will get an
object containing the JSON.

```javascript
const {MoonbaseDIDRegistry} = require('moon-did-registry')
```
...
const DidReg = new ethers.Contract(registryAddress, MoonbaseDIDRegistry.abi)
DidReg.connect(yourSignerOrProvider)
```

## On-chain vs. Off-chain

For on-chain interactions Moonbase has a built-in account abstraction that can be used regardless of whether the account
is a smart contract or a key pair. Any transaction has a `msg.sender` as the verified sender of the transaction.

Since each Moonbase transaction must be funded, there is a growing trend of on-chain transactions that are authenticated
via an externally created signature and not by the actual transaction originator. This allows for 3rd party funding
services, or for receivers to pay without any fundamental changes to the underlying Moonbase architecture.

These kinds of transactions have to be signed by an actual key pair and thus cannot be used to represent smart contract
based Moonbase accounts.
...
Any Moonbase account regardless of whether it's a key pair or smart contract based is considered to be an account
identifier.
...
Moonbase address, such as an advanced multi-signature contract.
...
current Moonbase implementations provide a very inefficient lookup mechanism.
...
the [ethrn-did-resolver](https://github.com/decentralized-identity/ethr-did-resolver/blob/master/doc/did-method-spec.md)
repository.
...
Copy the `senderAddress` and send `cost` amount of ether to that address on the Moonbase network you wish to deploy to.
